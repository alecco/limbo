...
impl DumbLruPageCache {
    ...
    pub fn resize(&mut self, capacity: usize) -> ResizeResult {
        ...
        match self.make_room_for(0) {
            Ok(_) => ResizeResult::Done,
            Err(_) => ResizeResult::PendingEvictions,
        }
    }
...
mod tests {
    ...
    fn test_insert_existing_key_fail() {
        ...
        assert!(page_has_content(&page1_v1)); // And stay intact
        cache.verify_list_integrity();
    }
    ...
    fn test_detach_with_active_reference_clean() {
        ...
        let page_ref = cache.get(&key).expect("Page should exist");
            ...
            match rng.next_u64() % 3 {
                0 => {
                    ...
                    match cache.insert(key.clone(), page.clone()) {
                        Err(CacheError::Full | CacheError::KeyExists | CacheError::ActiveRefs) => {} // Ignore
                        Err(err) => {
    ...
    fn test_resize_smaller_success() {
        ...
        assert_eq!(result, ResizeResult::Done);
        assert_eq!(cache.len(), 3);
    ...
    fn test_resize_with_active_references() {
        ...
        assert_eq!(cache.len(), 3);
        cache.verify_list_integrity();
        assert_eq!(cache.resize(2), ResizeResult::PendingEvictions);
        assert_eq!(cache.capacity, 2);
        assert_eq!(cache.len(), 3);
        drop(page2);
        drop(page3);
        assert_eq!(cache.resize(1), ResizeResult::Done); // Evicted 2 and 3
        assert_eq!(cache.len(), 1);
        assert!(cache.insert(create_key(4), page_with_content(4)).is_err());
        cache.verify_list_integrity();
    }
    ...
    fn test_resize_to_zero() {
        ...
        assert_eq!(result, ResizeResult::Done); // removed all 3
        assert_eq!(cache.len(), 0);
    ...
    fn test_resize_same_capacity() {
        ...
        let result = cache.resize(3);
        assert_eq!(result, ResizeResult::Done); // no-op
        assert_eq!(cache.len(), 3);
        assert_eq!(cache.capacity, 3);
        cache.verify_list_integrity();
